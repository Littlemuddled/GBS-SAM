    ...
    for epoch in range(num_epochs):
        for ent in tqdm(train_data_list, desc=f"Epoch {epoch + 1}/{num_epochs}", total=len(train_data_list)):
            ...
            with torch.cuda.amp.autocast():
                ...
                # 提取high_res_features并存入memory bank  TODO
                src_feat = high_res_features[-1].squeeze(0)  # [C, H, W]
                src_feat = F.adaptive_avg_pool2d(src_feat, 1).flatten()  # [C]
                src_feat = F.normalize(src_feat, p=2, dim=0)  # L2归一化
                source_feature_bank.append(src_feat.detach().cpu())
                ...
                for i in range(num_instances):
                    ...
                    source_prompt_bank.append(feat)
            ...
            if global_step % steps_per_epoch == 0:
                ...
                if val_dice > best_val_dice:
                    ...
                    torch.save(list(source_prompt_bank), os.path.join(save_dir, "best_val_prompt_bank.pt"))
                    torch.save(list(source_feature_bank), os.path.join(save_dir, "best_val_feature_bank.pt"))  # 保存memory bank
